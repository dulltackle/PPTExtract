...
- 摄取和发布任务保存在 PostgreSQL 中。持久任务 worker 使用 `FOR UPDATE SKIP LOCKED`、租约到期时间和尝试次数认领任务；交付语义为至少一次，因此每个阶段都必须按输入哈希、工具版本和稳定目标身份幂等。“持久”修饰任务、租约与检查点；worker 是可随时重启或替换的执行者，其进程内状态不是恢复依据。
