...
- 深色应用框架把产品身份、当前区域、操作者与键盘命令固定在连续工作面周围。
- 监看器黑与场记纸形成证据、决策双表面。
...
**The 证据可核验 Rule.** 来源部件、媒体类型、字节数、哈希完整性与像素尺寸等关键元数据可以紧凑，但必须保持长时间阅读与逐字核验所需的字号、对比和换行，不得为压缩栏宽降格为装饰性微字。

## Layout

布局以“证据区与决策区并置”为耐久原则：来源渲染占据可用空间的主位，页队列、日志或审核控制围绕它建立连续路径。单个工作台采用三栏并不意味着所有页面都必须复用同一比例；新表面应从任务所需的证据—决策关系推导构图。

桌面是当前核心环境。应用框架使用固定顶部状态条与底部命令条，中间区域独立滚动；当前真实壳层以 58px 顶栏和 48px 命令条夹住工作区，并在 1320px 触发桌面降密度调整。新表面可以按任务压缩页列、日志动作与来源画面边距，但必须保留核心内容的最小可读宽度。界面允许较高信息密度，并以紧凑、成组的间距节奏替代大块留白。

**The 证据不退场 Rule.** 编辑与结论界面必须让来源证据保持可见或可立即对照，不能把关键判断拆进失去上下文的通用卡片流程。

**The 构图非模板 Rule.** 复用证据与决策并置、桌面高密度及 1280px 级可用性，不复制某张已批准页面的固定栏宽。

**The 结论不漂移 Rule.** 长日志中的页面结论与结构性阻塞必须持续可见；它们应位于独立布局轨道，内容在自己的区域内滚动，不能依靠遮住表单或队列尾部的浮层来实现固定。
...
- **边界说明：**由触发控件锚定的短暂说明获得明确投影，帮助操作者理解尚未开放或受限的能力而不离开当前工作面。
- **恢复错误：**阻塞当前工作位的连接错误可以使用克制投影与红色发丝边界，但必须同时给出文字原因和恢复动作。
...
顶部状态条承载产品标记、当前区域、稳定操作者和表面级主动作；当前区域使用文字与细 teal 下划线共同标记。左侧队列承载页级移动，底部命令条持续展示真实可用的高频键盘动作和当前连接状态。当前筛选、页项和页签均通过边框、底色与文字共同标记；较窄桌面只做降密度，不切换移动端导航。

### Loading, Empty, and Recovery States

加载态维持最终布局骨架，以暗面短条和低速扫光表示等待，并在 `prefers-reduced-motion` 下停止动画。空态保留未来内容的结构基线，以图形、明确的“当前为空”文案和下一步来源共同解释；不得制造示例业务行。错误态留在应用壳层内，使用可聚焦的恢复动作，并让底部命令状态同步表达“需要恢复”。
...
- **引用队列：** AnyDoc 图片严格按页内固定引用顺序编号与处置；字节重复只作证据提示，不合并条目、决定或审计记录。
- **单项工作位：** 队列同时只展开一项，折叠行持续显示编号、来源身份与处置状态；保存后自动转入下一未完成项，维持长队列的连续键盘节奏。
- **决策分支：** “保留”始终在“忽略”之前，视觉排序、DOM 顺序与键盘焦点顺序完全一致；两个分支各自只展示当前决定需要的字段。
- **硬校验闸门：** 保留项缺少自足 summary，忽略项缺少稳定原因，“其他”缺少说明，或原始字节完整性无法核验时，不得保存该项、前进到下一项或完成来源审核；阻塞文案必须可聚焦并把焦点送回缺失字段。
- **原始资产语义：** 预览是对 AnyDoc 提供的原始图片字节的核验入口，不是缩略图或裁剪后代替品；选择保留时，界面必须明示该文件将以原始字节、原媒体类型与内容哈希进入产物，并在产物中作为 `anydoc_original` 证据；预览失败不得装作资产不存在，而应保留元数据与明确恢复途径。
...
- **Do** 让加载、空白、错误与恢复状态延续同一工作面，并诚实说明下一步来源。
...
- **Don't** 用虚构业务行填充尚无数据的表面，或让未开放动作表现为已经提交。
