...
- **真实性边界**：本票没有文档 API 领域流程，三条跑道必须由 bootstrap API 驱动并保持诚实空态；批准稿中的文档行只表达未来密度，不进入产品。上传入口响应操作并明确说明 #20 边界，但绝不发起提交。
...
| 上传边界 | 顶栏入口可聚焦、可响应；只展开可见边界说明，不发送请求 | React button + inline status |
...
- **STORY**：操作者带着稳定身份进入“文档”区域；顶栏下立即出现三条跑道；每条以真实 0 项说明当前没有可续接工作；上传入口说明 #20 的能力边界；底部只保留当前真实可用的刷新与焦点命令。
...
- **上传边界**：入口可聚焦、可展开并明确说明 #20；当前实现不接收文件、不发送上传请求。
